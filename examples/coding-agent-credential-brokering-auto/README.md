...
A coding agent example to work on the public [`vercel/ms`](https://github.com/vercel/ms) repository using automatic credential brokering.
...
- Configures `credentialBrokering: { mode: "auto" }` to replace sandbox-safe credentials in matching outbound requests.
